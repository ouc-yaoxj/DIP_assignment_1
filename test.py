...
from Noise_Reduction import add_gaussian_noise
...
img = cv2.imread('/mnt/4T/yxj/digital_image_processing/Assignment_1/image/RGB.jpg', 0)
# noise_image = add_white_noise(img, 40)
noise_image = add_gaussian_noise(img, mean=0, var=0.001)
cv2.imshow('bbb', noise_image)
noise_size = estimate_noise_amplitude(noise_image)
wiener_image = wiener(noise_image).astype(np.uint8)
# np.clip(wiener_image, 0, 255, out=wiener_image)

cv2.imshow('aaa', wiener_image)
cv2.waitKey(0)